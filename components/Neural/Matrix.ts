...
export class Matrix {
  ...
  add = ({ data: inData }: Matrix) =>
    this.map((val, i, j) => val + inData[i][j]);

  addNum = (n: number) => this.map((val) => val + n);

  multiply = ({ data: inData }: Matrix) =>
    this.map((val, i, j) => val * inData[i][j]);

  multiplyNum = (n: number) => this.map((val) => val * n);

  cpy = () => Matrix.create(this.rows, this.cols, (i, j) => this.data[i][j]);

  map = (otherFunction: (val: number, i: number, j: number) => number) => {
