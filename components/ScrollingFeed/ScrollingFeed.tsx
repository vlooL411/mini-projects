...
function ScrollingFeed<T>({
	...
	const UpdateInterval = (): void => {
		[interval.start, interval.end] = [
			interval.end,
			interval.end + stepInterval,
		];
		setInterval({ ...interval });
	};

	useEffect(() => {
		(onlyBody ? globalThis : scrollBody.current).onscroll = () => {
	...
	return (
		<>
			<div className={feed ?? feedDefault} style={style} ref={scrollBody}>
				...
				{isMaxDatas() ? (
					<div className={loaderDefault ?? loader}>
						{!loading && isScrollCollapse ? (
							<button onClick={UpdateInterval}>Load yet</button>
						) : null}
