...
const toesInit = (): TToe[] =>
	Array.from<unknown, TToe>({ length: 9 }, () => ({ char: null }));
...
const Tic_Tac_Toe = (): ReactElement => {
	...
	const [moves, setMoves] = useState<{ current?: boolean; firstMove: boolean }>(
		{ firstMove: randomMove() },
	);

	useEffect(() => {
		moves.current = moves.firstMove;
	}, [moves.firstMove]);
	...
	const GetMove = (move: boolean = moves.current): TToeChar =>
		move ? 'x' : 'o';
	const Win = useMemo(
		() =>
			win ? (
				<>
					Win
					<span className={current_move_char}>{GetMove(!moves.current)}</span>
				</>
			) : history.length == 9 ? (
				<>End</>
			) : null,
	...
	return (
		<>
			<p className={current_move}>
				Current move:
				<span className={current_move_char}>{GetMove()}</span>
			</p>
			<div className={game}>
			...
			<div className={tictac_win}>{Win}</div>
		</>
	);
};
